React On SharePoint Sample
==========================

This is the code I showed during my presentation on developing React Single Page Apps on SharePoint. This code will continue to be updated and modified as I tweek the presentation.  

To get this in a workable state make sure you have Node and NPM install as per the instructions on the Create-React-App, and then run the following:

```npm install```  
```npm start```

To build a copy to deploy to SharePoint, use  
```npm run build```

To deploy this to SharePoint, make sure you edit the ```package.json``` file.  Change the **homepage** setting to reflect the path to your application:

```
"homepage": "/sites/dev/siteassets/contactform"
```

Build the application using ```npm run build``` and then copy the files in the build folder to the SharePoint Site Assets library.  Place a Content Editor Web Part onto a page and reference the ```index.html``` file.
